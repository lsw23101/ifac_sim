...
def Enc_state(z_hat_bar, sk, env, T1, T2, V2):
    ...
    z_hat_bar_L = Mod(env.L * z_hat_bar, env.q)
...
def Dec(ciphertext, sk, env):
    ...
    # 1) 모듈러 곱 (0..q-1)
    m_bar_L = matmul_mod(ciphertext, one_sk_one, env.q)  # (h x 1)

    
    # 2) centered 로 다시 잡기 [-q/2, q/2)
    m_center = Mod(m_bar_L, env.q)   # 여기서 Mod는 네가 정의한 centered 함수

    m_bar = m_center / env.L
